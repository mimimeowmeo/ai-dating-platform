...
2026-09-23 起（PR #9），建立或編輯個人檔案都必須填至少 20 字的自我介紹。所以新使用者至少會落在「低信心、只用自我介紹」這一格；「無」只會出現在規則上線前、資料不完整的舊帳號。
...
- **什麼帳號會碰到「完全沒有依據」**：2026-09-23 起（PR #9），個人檔案必須填自我介紹（至少 20 字）、想遇見的關係 1～2 項，以及各類小熱愛標籤。填好的檔案一定有資料根據，所以這條規則主要保護資料不完整的舊帳號。
...
| 為什麼有時候只有 1～2 則，甚至一則都沒有？ | 系統不硬湊：找不到依據就少給；完全沒有資料根據時（通常是資料不完整的舊帳號），直接顯示「沒有可推薦的句子」。把個人檔案填完整，推薦就會有依據 |
...
- 相關 PR：#2（AI 服務）、#3（後端）、#4（前端）、#5（比例與不硬湊）、#7（tunnel 的 AI 位址）、#8（背景讓路）、#9（個人檔案必填欄位）
