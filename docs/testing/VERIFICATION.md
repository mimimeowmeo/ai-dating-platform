...
| `apps/web/tests/*.spec.ts`           | Playwright e2e：首頁與受保護頁面、註冊到探索的完整流程、雙人互讚與即時訊息、照片上傳不會清掉未儲存的輸入、多分頁不會登出、聊天歷史；真人驗證（沒照片時沒有入口、上傳後開鏡頭、領挑戰、MediaPipe 由本站提供、各原因代碼的說明）                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `GET /verification/status`                           | `verification_records` `user_photos`                                                    | —                                                                                            |
| `POST /onboarding/selfie`                            | `user_photos`（第一張主照片）`verification_records`                                     | `verification_records`、必要時 `users.is_verified`                                           |
| `POST /verification/challenge`                       | `user_photos`                                                                           | Redis `verify-challenge:<id>`（120 秒失效）                                                  |
| `POST /onboarding/live`                              | `user_photos`、Redis 挑戰（讀出即刪）`verification_records`                             | `verification_records`、必要時 `users.is_verified`                                           |
| `POST /verification/retry`                           | `user_photos`                                                                           | —（只判斷有沒有大頭貼，不寫資料庫）                                                          |
...
| `/verification`       | 開始真人驗證                             | `POST /verification/challenge` → 拍完 `POST /onboarding/live` → invalidate `/verification/status`、`GET /auth/me` |
...
| 項目                                                                                      | 影響                                                               |
| ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| `POST /verification/retry` 只看有沒有大頭貼，不看最近一筆驗證紀錄                         | `status` 永遠是 `not_started`，可能與 `GET /verification/status` 的狀態不同（`canVerify` 一致）；畫面目前沒有入口 |
| 註冊頁「我已滿 18 歲」checkbox 沒有接 state                                               | 勾選狀態不會送出，只靠瀏覽器原生 `required` 擋                     |
