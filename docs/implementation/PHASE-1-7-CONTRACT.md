...
- Photo：`id,url,isAvatar,displayOrder`；`POST /profile/photos` multipart `file` → Photo；`DELETE /profile/photos/:photoId` → `{ok:true}`，軟刪除：照片記錄與物件保留，之後不會出現在任何回應，舊的媒體網址回 404；刪掉主照片時由下一張接替。
